...
export class ClashService {
  ...
  get proxies() {
    return this._proxies$.pipe(filter(data => !!data));
  }
  get rules() {
    return this._rules$.pipe(filter(data => !!data));
  }
  get traffic() {
    return this._traffic$.pipe(filter(data => !!data.length));
  }
  get logs() {
  ...
  delay(name: string, timeout: number = 5000, url: string = 'http://www.gstatic.com/generate_204') {
    return this.http
      .get(`${this.url}/proxies/${name}/delay`, { timeout, url }, { headers: this.headers, observe: 'response' })
      .pipe(
        tap(
          x => {
            console.log('delay', x);
  ...
  setSelector(proxy: string, name: string) {
    return this.http
      .put(`${this.url}/proxies/${proxy}`, { name }, null, { headers: this.headers, observe: 'response' })
      .pipe(
        tap(
          x => {},
          err => {
  ...
  setEc(v: IApiInfo) {
    return this.http
      .get(`http://${v.host}:${v.port ? v.port : 80}/configs`, null, {
  ...
  getProxies() {
    return this.http.get<IProxies>(`${this.url}/proxies`, null, { headers: this.headers, observe: 'body' }).pipe(
      map(r => r.proxies || {}),
      tap(
        data => {
  ...
  getRules() {
    return this.http.get<IRules>(`${this.url}/rules`, null, { headers: this.headers, observe: 'body' }).pipe(
      map(r => r.rules || []),
      tap(
        data => {
        ...
        r => {
          if (this.trafficCtrl.need) {
            ...
            if (len > this._ec.numTraffic) {
              this._traffic$.next(
                [...this.trafficCtrl.holdValue.slice(r.length - this._ec.numTraffic), ...r].slice(-5),
              );
            } else {
